...
"""
This file is from Suraj Patil's question_generation repository:
https://github.com/patil-suraj/question_generation
I only added comments here and there and made a modification
in line 155/156 as suggested in the issue section of the
repository.
"""
...
class QGPipeline:
    ...
    def __call__(self, inputs: str):
        inputs = " ".join(inputs.split())      # alpha: but why???
    ...
    def _prepare_inputs_for_ans_extraction(self, text):
        sents = sent_tokenize(text)   # segment text into sentences

        inputs = []
        for i in range(len(sents)):   # Iteration ü Anzahl Sätze
            source_text = "extract answers:"
            for j, sent in enumerate(sents):
                if i == j:  # packe gesamten Satz in highlight-Klammern
                    sent = "<hl> %s <hl>" % sent
                source_text = "%s %s" % (source_text, sent)
                source_text = source_text.strip()
            # am Ende dieser zweiten For-Schleife ist in der Variable sent der
            # gehighlightete Satz gespeichert und in source_text Folgendes:
            # "extract asnwers: Satz1,Satz2,...,Satzn", wobei der Satz mit dem
            # Index der äußeren Schleife gehighightet ist.
